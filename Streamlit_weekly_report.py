...
def MonteCarlo(simulation_number=10000):
    """Run Monte Carlo simulations of bitcoin prices."""
    prices = {}
...
def count_simulations(result, price_level, price0):
    """Count the number of simulations in which bitcoin reaches a certain price level."""
    if price_level < price0:
        return (result <= price_level).any(axis=0).sum()
    else:
        return (result >= price_level).any(axis=0).sum()

# Run Monte Carlo simulations and calculate probabilities
result = MonteCarlo()
price_levels = [5000, 10000, 15000, 20000, 25000, 30000, 35000, 50000, 70000, 100000]
probabilities = []
...
table = pd.DataFrame({
    "Price Level": ["${0:,.0f}".format(price) for price in price_levels], # add $ and format prices
